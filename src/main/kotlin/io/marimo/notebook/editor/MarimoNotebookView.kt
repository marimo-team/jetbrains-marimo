 ...
 * panel, and the marimo server it renders. Owned by [MarimoServerService] and keyed by file, not
...
class MarimoNotebookView(private val project: Project, private val file: VirtualFile) : Disposable {
    ...
    val panel: JPanel = object : JPanel(BorderLayout()), DataProvider {
    ...
    val preferredFocusedComponent: JComponent? get() = browser?.component
     ...
     * failure; release first to force a fresh launch. The browser is kept and reused — pointing it at the
     * fresh server URL is exactly the restart the user asked for.
     */
