 ...
 * without pip, so those fall back to `uv pip install --python <interpreter> marimo`. Relaunching
 * the editor after a successful install is the caller's responsibility.
 */
@Service(Service.Level.PROJECT)
class MarimoInstaller(private val project: Project) {

    /**
     * Show modal install progress for marimo on [notebook]'s interpreter and return the freshly
     * probed presence. Returns [MarimoPresence.Unknown] when no interpreter is configured to
     * install into. Must be called on the EDT; the install and the re-probe run off it, inside the
     * modal progress task.
     */
    fun installMarimo(notebook: VirtualFile): MarimoPresence {
        ...
        val presence =
            try {
                ProgressManager.getInstance()
                    .runProcessWithProgressSynchronously<MarimoPresence, RuntimeException>(
                        {
        ...
        MarimoTelemetry.getInstance()
            .capture(
                TelemetryEvent.MarimoInstallResult(success = presence is MarimoPresence.Installed)
            )
        return presence
    }

    /**
     * The command that installs marimo into [pythonPath]'s environment, or null when neither pip
     * nor uv can reach it. Runs a subprocess to detect pip, so call it off the EDT.
     */
    private fun resolveInstallCommand(pythonPath: String): GeneralCommandLine? =
        when {
    ...
    private fun hasPip(pythonPath: String): Boolean {
        ...
        val output = runCatching {
            CapturingProcessHandler(cmd).runProcess(PIP_CHECK_TIMEOUT_MS)
        }
            .getOrNull()
