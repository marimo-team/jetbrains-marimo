...
class MarimoNotebookLifecycle(
    ...
    fun onShutdownObserved() {
        val gen = synchronized(lock) { generation }
        val moved = setState(gen) {
            (state as? MarimoNotebookState.Running)?.let { MarimoNotebookState.Stopping(it.url) }
        }
    ...
    private fun onReady(gen: Long, url: String) {
        setState(gen) { if (state is MarimoNotebookState.Starting) MarimoNotebookState.Running(url) else null }
    }

    private fun onLaunchFailed(gen: Long, error: Throwable) {
        setState(gen) { if (state is MarimoNotebookState.Starting) MarimoNotebookState.Failed(error) else null }
    }
    ...
    private fun onProcessTerminated(gen: Long, exitCode: Int, outputTail: String) {
        setState(gen) {
            when (state) {
                ...
                is MarimoNotebookState.Stopped, is MarimoNotebookState.Failed -> null
    ...
    /**
     * Intent was observed, so the page is dead whether or not the process is: marimo may have closed
     * just this session. Restarting from here releases the process first, so claiming stopped while
     * it lingers is safe.
     */
     ...
     * listeners outside the lock. Returns true when a transition happened. A null from [next]
