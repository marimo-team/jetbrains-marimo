 ...
 * project's `pyproject.toml`, the notebook's own PEP 723 metadata, environment overrides — and inlines
 * the result into the page as `<marimo-user-config data-config="…">`. Asking the server for that answer
 * keeps the plugin from reimplementing (and drifting from) the resolution rules.
 */
object MarimoPageConfig {

    /**
     * The effective `display.theme` ("light", "dark", "system", …) for the notebook served at [url],
     * or null when the page or the field can't be read. Blocking; call off the EDT.
     *
     * The two failure modes are not equally interesting. An unreachable or unreadable server is
     * environmental and expected (the process can die between readiness and this request), so it is only
     * logged. A page that *was* fetched but carries no theme means this code no longer understands what
     * marimo serves — the theme silently stops following the IDE — so that one is reported.
     */
    ...
    /**
     * Carries no URL or page content: the failure is that marimo's served config could not be located or
     * parsed, and the stack trace is the actionable part.
     */
